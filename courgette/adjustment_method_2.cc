...
//   base::StringAppendF(&s, " #%u", info->refs_);
  ...
  char temp[200] = "";
  sprintf_s(temp, sizeof(temp), "%c%d", "pm"[info->is_model_], info->debug_index_);
  s += temp;

  if (info->label_->index_ != Label::kNoIndex)
  {
      sprintf_s(temp, sizeof(temp), " (%d)", info->label_->index_);
      s += temp;
  }

  sprintf_s(temp, sizeof(temp), " #%u", info->refs_);
  s += temp;
...
//       printf("score %d  %s:= ?\n", TopScore(), ToString(program_info_).c_str());
//       if (!pending_updates_.empty())
//           printf("%d pending\n", pending_updates_.size());
//       int count = 0;
//       for (Queue::iterator q = queue_.begin();  q != queue_.end();  ++q) {
//           if (++count > max) break;
//           printf("   %s\n", ToString(q->second).c_str());
//       }
  }
...
//                       instance->position_count());
  ...
  const char* sep = "<";
  for (size_t i = 0; i < Shingle::kWidth; ++i) {
    sprintf_s(temp, sizeof(temp), "%s%x ", sep, instance->at(i)->label_->rva_);
    s += temp;
    s += sep;
    sprintf_s(temp, sizeof(temp), "%d ", instance->at(i));
    s += temp;
    sep = ", ";
  }
...
//   }
  if (index == NULL) {
    ...
    char temp[200] = "";
    printf_s(temp, sizeof(temp), "<%d: ", index->variables_);
    s += temp;
    const char* sep = "";
    for (size_t i = 0;  i < Shingle::kWidth;  ++i) {
      s += sep;
      ...
      int offset = kind & ShinglePattern::kOffsetMask;
      if (kind & ShinglePattern::kVariable)
        printf_s(temp, sizeof(temp), "V%d", offset);
      else
        printf_s(temp, sizeof(temp), "%d", index->assigned_indexes_[offset]);
      s += temp;
     }
    printf_s(temp, sizeof(temp), " %x>", index->hash_);
    s += temp;
  }
  return s;
}
...
std::string HistogramToString(const ShinglePattern::Histogram& histogram,
  ...
  for (ShinglePattern::Histogram::const_iterator p = histogram.begin();
    ...
    char temp[200] = "";
    printf_s(temp, sizeof(temp), " %d", p->count());
    s += temp;
  }
  return s;
}
...
//   s += HistogramToStringFull(pattern->program_histogram_, "    ", max);
  ...
  size_t program_size = pattern->program_histogram_.size();
  char temp[200] = "";
  printf_s(temp, sizeof(temp), "  model shingles %d\n", model_size);
  s += temp;
  s += HistogramToStringFull(pattern->model_histogram_, "    ", max);
  printf_s(temp, sizeof(temp), "  program shingles %d\n", program_size);
  s += temp;
  s += HistogramToStringFull(pattern->program_histogram_, "    ", max);
  return s;
}
...
//             << trace.size();
     printf("AssignmentProblem::AssignmentProblem  %d, %d\n", model_end , trace.size());
  }
...
//             << active_non_single_use_patterns_.size() << " active patterns";
     printf("instances:%d trace length:%d shingle indexes:%d single use patterns:%d active patterns:%d\n",
         shingle_instances_.size(),
         trace_.size(),
         patterns_.size(),
         single_use_pattern_queue_.size(),
         active_non_single_use_patterns_.size());
  }
  ...
  void PrintAllShingles() const {
    for (Shingle::OwningSet::const_iterator p = shingle_instances_.begin();
      ...
      //VLOG(2) << ToString(&instance) << "   " << ToString(instance.pattern()) << std::endl;
    }
  }
...
//             << (base::Time::Now() - start_time).InSecondsF();
    ...
    printf(" Adjuster::Solve %d millisecond\n", (GetTickCount() - start_time));
  }
