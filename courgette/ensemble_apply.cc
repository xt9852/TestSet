...
//     return C_READ_OPEN_ERROR;
  ...
  fopen_s(&patch_file, patch_file_name, "rb");

  if (NULL == patch_file) return C_READ_OPEN_ERROR;

  fseek(patch_file, 0, SEEK_END);

  __int64 patch_file_length = ftell(patch_file);

  unsigned char *patch_file_data = new unsigned char[patch_file_length];

  fseek(patch_file, 0, SEEK_SET);

  size_t len = fread(patch_file_data, 1, patch_file_length, patch_file);

  fclose(patch_file);

  // 'Dry-run' the first step of the patch process to validate format of header.
  SourceStream patch_header_stream;
  patch_header_stream.Init(patch_file_data/*patch_file.data()*/, patch_file_length/*patch_file.length()*/);
  EnsemblePatchApplication patch_process;
...
//     return C_READ_ERROR;
  ...
  fopen_s(&old_file, old_file_name, "rb");

  if (NULL == old_file) return C_READ_OPEN_ERROR;

  fseek(old_file, 0, SEEK_END);

  __int64 old_file_length = ftell(old_file);
  ...
  old_source_stream.Init(old_file_data/*old_file.data()*/, old_file_length/*old_file.length()*/);
  patch_source_stream.Init(patch_file_data/*patch_file.data()*/, patch_file_length/*patch_file.length()*/);
...
begin:
  ...
  fopen_s(&new_file, new_file_name, "wb");
