...
bool DisassemblerWin32X86::Disassemble(AssemblyProgram* target) {
  ...
  if (!ParseAbs32Relocs()) // 扫描基址重定位表
    return false;

  ParseRel32RelocsFromSections(); // 扫描代码段
...
//           ..
//   IMAGE_BASE_RELOCATION;     // 有多个重定位结构
// VirtualAddress:
// 是这一组重定位数据的开始RVA地址.各重定位项的地址加上这个值才是该重定位项完整的RVA地址.
// SizeOfBlock:
// 是重定位结构的大小
// TypeOffset:
// 是一个数组.数组每项大小为两个字节,共16位.它又分为高4位和低12位,高4位代表重定位类型;
// 低12位是重定位地址,它与VirtualAddress相加即是指向PE映像中需要修改的地址数据的指针.
