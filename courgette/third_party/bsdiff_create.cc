...
//   base::Time start_bsdiff_time = base::Time::Now();
  uint32 start_bsdiff_time = GetTickCount();
//  VLOG(1) << "Start bsdiff";
  printf("Start bsdiff\n");
...
//                << " bytes"
    printf("Could not allocate I[], %d bytes\n", ((oldsize + 1) * sizeof(int)));
    return MEM_ERROR;
  }
...
//                << " bytes"
     printf("Could not allocate V[], %d bytes\n", ((oldsize + 1) * sizeof(int)));
     return MEM_ERROR;
  }

//  base::Time q_start_time = base::Time::Now();
  uint32 q_start_time = GetTickCount();
  qsufsort(I, V, old, oldsize);
//   VLOG(1)<< " done qsufsort "
//           << (base::Time::Now() - q_start_time).InSecondsF();
  printf(" done qsufsort %d millisecond\n", (GetTickCount() - q_start_time));
  V.clear();
...
//           << (base::Time::Now() - start_bsdiff_time).InSecondsF();
  ...
  printf("Control tuples: %d  copy bytes: %d  mistakes: %d (skips: %d) extra bytes: %d\nUncompressed bsdiff patch size %d\nEnd bsdiff\n",
         control_length,
         diff_bytes_length,
         diff_bytes_nonzero,
         diff_skips_length,
         extra_bytes_length,
         patch_stream->Length() - initial_patch_stream_length,
         (GetTickCount() - start_bsdiff_time));
