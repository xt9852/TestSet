...
//  base::Time start_find_time = base::Time::Now();
  uint32 start_find_time = GetTickCount();
...
//            << (base::Time::Now() - start_find_time).InSecondsF();
  printf("done FindEmbeddedElements %d millisecond\n",
      (GetTickCount() - start_find_time));
...
//  base::Time start_bases_time = base::Time::Now();
  uint32 start_bases_time = GetTickCount();
...
//           << (base::Time::Now() - start_bases_time).InSecondsF() << "s";
  printf("done make bases %d millisecond\n", (GetTickCount() - start_bases_time));
...
//      base::Time start_compare = base::Time::Now();
      uint32 start_compare = GetTickCount();
...
//               << "s";
      printf("Compare %s to %s -> %d in %d millisecond\n",
          old_element->Name().c_str(),
          new_element->Name().c_str(),
          difference,
          (GetTickCount() - start_compare));

      if (difference == 0) {
//         VLOG(1) << "Skip " << new_element->Name()
//                 << " - identical to " << old_element->Name();

        printf("Skip %s - indentical to %s\n",
            new_element->Name().c_str(),
            old_element->Name().c_str());
...
//           << "s";
  ...
  printf("done FindGenerators found  %d int %d millisecond\n",
      generators->size(),
      (GetTickCount() - start_find_time));
...
Status GenerateEnsemblePatch(SourceStream* base,
  ...
  printf("start GenerateEnsemblePatch\n");
...
//           << (base::Time::Now() - start_time).InSecondsF() << "s";
  ...
  printf("done GenerateEnsemblePatch %d millsecond\n", (GetTickCount() - start_time));
  return C_OK;
}
