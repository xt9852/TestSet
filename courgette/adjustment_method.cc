...
//   s += "}";
  ...
  printf(temp, sizeof(temp), "%u", node->count_);
  s += temp;
  s += " @";
  printf(temp, sizeof(temp), "%I64d", node->edges_in_frequency_order.size());
  s += temp;
  s += "}";
  return s;
}
 ...
 private:
  void AddToQueue(Node* node) {
    if (node->length_ >= 10) {
      ...
      printf("Length clipped %s\n", ToString(node->prev_).c_str());
      return;
    }
    if (node->in_queue_) {
  ...
  void SkipCommittedLabels(Node* node) {
    ...
    if (skipped > 0)
      ...
      printf("Skipped %d at %s\n", skipped, ToString(node).c_str());
  }
...
//               << ToString(p_match) << " vs " << ToString(m_match);
      printf("Tricky distribution %d:%d  %s vs %s\n",
          p_match->count_,
          m_match->count_,
          ToString(p_match).c_str(),
          ToString(m_match).c_str());
      return;
    }
...
//               << ToString(p_info_next) << " := " << ToString(m_info_next);
      printf("  Extending assignment -> %s := %s\n",
          ToString(p_info_next),
...
//               << "] <- " << ToString(p_info) << " := " << ToString(m_info);
      printf("    Extending assignment seq[-%s] <- %s := %s\n",
          (p_pos_start - p_pos),
          ToString(p_info),
          ToString(m_info));
