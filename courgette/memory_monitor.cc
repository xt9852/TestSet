...
//     for (M::iterator p = m_.begin(); p != m_.end(); ++p, ++i) {
//         size_t s = p->first;
//         sprintf_s(temp, sizeof(temp), "%3d %8u: %8u %8u %8u %8u", i, s, m_[s], c_[s], h_[s], h_[s] * s);
//         printf(temp);
//     }
// 
//    sprintf_s(temp, sizeof(temp), "Peak %s\n", fmt(high_).c_str());
  }
...
//     return base::StringPrintf("%d", (int)s);
      ...
      if (s > 1000000000) sprintf_s(intToString, sizeof(intToString), "%.3gG", s/(1000000000.0));
      else if (s > 1000000) sprintf_s(intToString, sizeof(intToString), "%.3gM", s/(1000000.));
      else if (s > 9999) sprintf_s(intToString, sizeof(intToString), "%.3gk", s/(1000.));
      else sprintf_s(intToString, sizeof(intToString), "%d", (int)s);
...
//     base::StringAppendF(&o, " (%c%s)", sign, fmt(w).c_str());
    ...
    char temp[500] = "";
    sprintf_s(temp, sizeof(temp), "%u:", tick_ + d_tick_);
    o += temp;
    sprintf_s(temp, sizeof(temp), " (%c%s)", sign, fmt(w).c_str());
    o += temp;
    ...
    for (M::iterator p = c_.begin(); p != c_.end(); ++p) {
      ...
      if (n) {
        if (s*n >= 64*1024)
          ...
          {
              sprintf_s(temp, sizeof(temp), "  %s", fmt(s).c_str());
              o += temp;
          }
          else
            //base::StringAppendF(&o, "  %s*%u", fmt(s).c_str(), n);
          {
              sprintf_s(temp, sizeof(temp), "  %s*%u", fmt(s).c_str(), n);
              o += temp;
          }
        sum += s*n;
        }
...
//     LOG(INFO) << o;
    ...
    sprintf_s(temp, sizeof(temp), "  = %s", fmt(sum).c_str());
    o += temp;
