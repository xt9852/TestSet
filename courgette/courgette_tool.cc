...
std::string ReadOrFail(const char* /*base::FilePath&*/ file_name, const char* kind) {
//   int64 file_size = 0;
...
//   return buffer;
    ...
    for(; len = fread(buf, 1, sizeof(buf), file); )
    {
...
void TokenizeData(const char *srcData, const char *tokenize, VECTOR_STR &data)
{
	...
	strcpy_s(temp, ARRAYSIZE(temp), srcData);

    char *next_token = NULL;
	char *token = strtok_s(temp, tokenize, &next_token);

	while (token != NULL)
	{
		data.push_back(token);
		token = (char*)strtok_s(NULL, tokenize, &next_token);
	}
}
...
void MakeDirectory(LPCSTR path)
{
    char temp[MAX_PATH] = "";
    VECTOR_STR data;
    TokenizeData(path, "\\", data);

    for (uint32 i = 0; i < (data.size() - 1); i++)
    {
        strcat_s(temp, ARRAYSIZE(temp), data[i].c_str());
        strcat_s(temp, ARRAYSIZE(temp), "\\");
...
void WriteSinkToFile(const courgette::SinkStream *sink, 
                     const char* /*base::FilePath&*/ output_file) {
//   int count =
...
begin:
  ...
  if (NULL == file)
  {
      ...
      {
          courgette::MakeDirectory(output_file);

          goto begin;
      }
      else
      {
          Problem("Can't open %s file.", output_file);
      }
  }

  int count =fwrite(reinterpret_cast<const char*>(sink->Buffer()), 
      1,
      static_cast<int>(sink->Length()), 
      file);
...
void Disassemble(const char* /*base::FilePath&*/ input_file,
                 const char* /*base::FilePath&*/ output_file) {
...
bool Supported(const char* /*base::FilePath&*/ input_file) {
  bool result = false;
...
void DisassembleAndAdjust(const char* /*base::FilePath&*/ program_file,
                          const char* /*base::FilePath&*/ model_file,
                          const char* /*base::FilePath&*/ output_file) {
...
void DisassembleAdjustDiff(const char* /*base::FilePath&*/ model_file,
                           const char* /*base::FilePath&*/ program_file,
                           const char* /*base::FilePath&*/ output_file_root,
                           bool adjust) {
  ...
  for (int i = 0;  ; ++i) {
    ...
    char intToString[64] = "";
    sprintf_s(intToString, sizeof(intToString), "%d", i);

    std::string append = output_file_root + std::string("-") + intToString/*base::IntToString(i)*/;

    WriteSinkToFile(&patch_stream,
                    append.c_str()/*output_file_root.InsertBeforeExtensionASCII(append)*/);
  }
}

void Assemble(const char* /*base::FilePath&*/ input_file,
              const char* /*base::FilePath&*/ output_file) {
  std::string buffer = ReadOrFail(input_file, "input");
...
void GenerateEnsemblePatch(const char* /*base::FilePath&*/ old_file,
                           const char* /*base::FilePath&*/ new_file,
                           const char* /*base::FilePath&*/ patch_file) {
...
void ApplyEnsemblePatch(const char* /*base::FilePath&*/ old_file,
                        const char* /*base::FilePath&*/ patch_file,
                        const char* /*base::FilePath&*/ new_file) {
...
void GenerateBSDiffPatch(const char* /*base::FilePath&*/ old_file,
                         const char* /*base::FilePath&*/ new_file,
                         const char* /*base::FilePath&*/ patch_file) {
  std::string old_buffer = ReadOrFail(old_file, "'old' input");
  std::string new_buffer = ReadOrFail(new_file, "'new' input");
...
void ApplyBSDiffPatch(const char* /*base::FilePath&*/ old_file,
                      const char* /*base::FilePath&*/ patch_file,
                      const char* /*base::FilePath&*/ new_file) {
  std::string old_buffer = ReadOrFail(old_file, "'old' input");
  std::string patch_buffer = ReadOrFail(patch_file, "'patch' input");
...
bool HasSwitch(int argc, const char* argv[], const char* item)
{
    for (int i = 0; i < argc; i++)
    {
        if (0 == strcmp(argv[i], item))
        {
            return true;
        }
    }
...
//   HMODULE module = LoadLibrary(L"courgette.dll");
// 
//   typedef int (*GetFilePatch)(const WCHAR *oldFile, const WCHAR *newFile, const WCHAR *patchFile);
// 
//   GetFilePatch getFilePatch = (GetFilePatch)GetProcAddress(module, "GenFilePatch");
// 
//   int ret = getFilePatch(L"D:\\OpenCode\\chrome\\courgette\\build\\DCMainWin_old.exe", 
//       L"D:\\OpenCode\\chrome\\courgette\\build\\DCMainWin_new.exe", 
//       L"D:\\OpenCode\\chrome\\courgette\\build\\file.patch");
// 
//   typedef int (*ApplyFilePatch)(const WCHAR *oldFile, const WCHAR *patchFile, const WCHAR *newFile);
//   
//   ApplyFilePatch applyFilePatch = (ApplyFilePatch)GetProcAddress(module, "ApplyFilePatch");
// 
//   ret = applyFilePatch(L"D:\\OpenCode\\chrome\\courgette\\build\\DCMainWin_old.exe", 
//       L"D:\\OpenCode\\chrome\\courgette\\build\\file.patch",
//       L"D:\\OpenCode\\chrome\\courgette\\build\\new_file.exe");
// 
// 
//   typedef int (*GetPePatch)(const WCHAR *oldFile, const WCHAR *newFile, const WCHAR *patchFile);
// 
//   GetPePatch getPePatch = (GetPePatch)GetProcAddress(module, "GenPEPatch");
// 
//   ret = getPePatch(L"D:\\OpenCode\\chrome\\courgette\\build\\DCMainWin_old.exe", 
//       L"D:\\OpenCode\\chrome\\courgette\\build\\DCMainWin_new.exe", 
//       L"D:\\OpenCode\\chrome\\courgette\\build\\pe.patch");
// 
//   typedef int (*ApplyPePatch)(const WCHAR *oldFile, const WCHAR *patchFile, const WCHAR *newFile);
// 
//   ApplyPePatch applyPePatch = (ApplyPePatch)GetProcAddress(module, "ApplyPEPatch");
// 
//   ret = applyPePatch(L"D:\\OpenCode\\chrome\\courgette\\build\\DCMainWin_old.exe", 
//       L"D:\\OpenCode\\chrome\\courgette\\build\\pe.patch",
//       L"D:\\OpenCode\\chrome\\courgette\\build\\new_pe.exe");

  bool cmd_sup = /*command_line.*/HasSwitch(argc, argv, "-supported");
  bool cmd_dis = /*command_line.*/HasSwitch(argc, argv, "-dis");
  bool cmd_asm = /*command_line.*/HasSwitch(argc, argv, "-asm");
  bool cmd_disadj = /*command_line.*/HasSwitch(argc, argv, "-disadj");
  bool cmd_make_patch = /*command_line.*/HasSwitch(argc, argv, "-gen");
  bool cmd_apply_patch = /*command_line.*/HasSwitch(argc, argv, "-apply");
  bool cmd_make_bsdiff_patch = /*command_line.*/HasSwitch(argc, argv, "-genbsdiff");
  bool cmd_apply_bsdiff_patch = /*command_line.*/HasSwitch(argc, argv, "-applybsdiff");
  bool cmd_spread_1_adjusted = /*command_line.*/HasSwitch(argc, argv, "-gen1a");
  bool cmd_spread_1_unadjusted = /*command_line.*/HasSwitch(argc, argv, "-gen1u");
