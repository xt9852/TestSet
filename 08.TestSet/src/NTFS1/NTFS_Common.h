...
typedef void (*SUBENTRY_CALLBACK_Ex)(const CIndexEntry *ie,PVOID pControl,lstp::basicutils::FileInfoItem* curParentItem,lstp::basicutils::ITEMTYPE& itemType,lstp::basicutils::FileInfoItem* &dirInfoItem,BOOL& bUsed,uint64& updateSize,FILETIME& lastChangeTime);
typedef void (*UpdateItem_CallBack)(lstp::basicutils::FileInfoItem* curParentItem,const void* pControl,uint64 updateSize, uint32 updateFileCount, uint32 updateDirCount, FILETIME& lastChangeTime);
typedef BOOL (*IsStopScan_CallBack)(const void* pControl);
// List Entry
