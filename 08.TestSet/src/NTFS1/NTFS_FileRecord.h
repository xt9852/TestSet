...
private:
	...
	BOOL TraverseSubNodeEx(const ULONGLONG &vcn, SUBENTRY_CALLBACK_Ex seCallBack,PVOID pContorl,lstp::basicutils::FileInfoItem* curParentItem,UpdateItem_CallBack udCallBack,IsStopScan_CallBack isCallBack) const;
...
public:
	...
	BOOL TraverseSubEntriesEx(SUBENTRY_CALLBACK_Ex seCallBack,PVOID pControl, lstp::basicutils::FileInfoItem* curNodeItem,UpdateItem_CallBack udCallBack,IsStopScan_CallBack isCallBack) const;
...
BOOL CFileRecord::TraverseSubNodeEx(const ULONGLONG &vcn, SUBENTRY_CALLBACK_Ex seCallBack,PVOID pContorl,lstp::basicutils::FileInfoItem* curParentItem,UpdateItem_CallBack udCallBack,IsStopScan_CallBack isCallBack) const
{
	CAttr_IndexAlloc *ia = (CAttr_IndexAlloc*)FindFirstAttr(ATTR_TYPE_INDEX_ALLOCATION);
	if (ia == NULL)
		return FALSE;

	uint64 totalChildFileSize = 0;
	uint32 totalChildFileCount = 0, totalChildDirCount = 0;
	FILETIME lastChangeTime = {0, 0};
	...
	if (ia->ParseIndexBlock(vcn, ib))
	{
		...
		while (ie)
		{
			if(isCallBack(pContorl))
				return FALSE;
			lstp::basicutils::ITEMTYPE tempType;
			lstp::basicutils::FileInfoItem* dirInfoItem = NULL;
			BOOL bUsed = FALSE;
			if (ie->HasName())
			{			
				uint64 updateSize = 0;
				FILETIME changeTime;
				memset(&changeTime,0,sizeof(FILETIME));
				seCallBack(ie,pContorl,curParentItem,tempType,dirInfoItem,bUsed,updateSize,changeTime);
				if(bUsed && tempType == lstp::basicutils::IT_FILE)
				{
					totalChildFileCount += 1;
					totalChildFileSize += updateSize;
					if(CompareFileTime(&lastChangeTime,&changeTime) < 0)
					{
						lastChangeTime = changeTime;
					}
				}
				else if(bUsed && tempType == lstp::basicutils::IT_DIRECTORY)
				{
					totalChildDirCount += 1;
					if(CompareFileTime(&lastChangeTime,&changeTime) < 0)
					{
						lastChangeTime = changeTime;
					}
			...
			if (ie->IsSubNodePtr() && bUsed && tempType == lstp::basicutils::IT_DIRECTORY)
			{
				if(dirInfoItem != NULL)
					TraverseSubNodeEx(ie->GetSubNodeVCN(), seCallBack,pContorl,dirInfoItem,udCallBack,isCallBack);	// recursive call
			}

			ie = ib.FindNextEntry();
		}
		udCallBack(curParentItem,pContorl,totalChildFileSize,totalChildFileCount,totalChildDirCount,lastChangeTime);
...
BOOL  CFileRecord::TraverseSubEntriesEx(SUBENTRY_CALLBACK_Ex seCallBack,PVOID pControl,lstp::basicutils::FileInfoItem* curNodeItem,UpdateItem_CallBack udCallBack,IsStopScan_CallBack isCallBack) const
{
	_ASSERT(seCallBack);
	...
	while (ie)
	{
		...
		{	
			...
			FILETIME changeTime;
			memset(&changeTime,0,sizeof(FILETIME));
			seCallBack(ie,pControl,curNodeItem,tempType,dirInfoItem,bUsed,updateSize,changeTime);
			if(bUsed && tempType == lstp::basicutils::IT_FILE)
			{
				totalChildFileCount += 1;
				totalChildFileSize += updateSize;
		...
		{			
			if(dirInfoItem != NULL)
				TraverseSubNodeEx(ie->GetSubNodeVCN(), seCallBack,pControl,dirInfoItem,udCallBack,isCallBack);
		}
	...
	udCallBack(curNodeItem,pControl,totalChildFileSize,totalChildFileCount,totalChildDirCount,lastChangeTime);
	return TRUE;
}
...
__inline const BOOL CFileRecord::FindSubEntry(const _TCHAR *fileName, CIndexEntry &ieFound) const
{
	...
	while (ie)
	{
		if (ie->HasName())
		{
			...
			else if (i < 0)		// fileName is smaller than IndexEntrys
			{
				// Visit SubNode
				if (ie->IsSubNodePtr())
				{
