...
void CKademliaUDPListener::SendMyDetails(byte byOpcode, uint32 uIP, uint16 uUDPPort, bool bKad2)
{
	if(bKad2)
	{
		...
		byteIOResponse.WriteUInt16(CKademlia::GetPrefs()->GetTcpPort());
	...
	else
	{
		...
		fileIO.WriteUInt16(CKademlia::GetPrefs()->GetUdpPort());
		//fileIO.WriteUInt16(thePrefs.GetPort());
		fileIO.WriteUInt16(CKademlia::GetPrefs()->GetTcpPort());
		fileIO.WriteUInt8(0);
...
//		if (thePrefs.GetDebugClientKadUDPLevel() > 0)
			DebugSendF("KADEMLIA_RES", uIP, uUDPPort, "Count=%u", uCount);
...
//		if (thePrefs.GetDebugClientKadUDPLevel() > 0)
//			DebugSendF("KADEMLIA2_RES", uIP, uUDPPort, "Count=%u", uCount);
...
SSearchTerm* CKademliaUDPListener::CreateSearchExpressionTree(CSafeMemFile& fileIO, int iLevel)
{
	...
	else if (uOp == 0x01) // String
	{
		...
		TokenizeOptQuotedSearchTerm((LPCTSTR)str.GetString(), pSearchTerm->m_pastr);
	...
	else if(uLenPacket > 17)
	{
		...
		if (uRestrictive)
		{
			try
			{
				...
				_pstrDbgSearchExpr = new CString;//(thePrefs.GetDebugServerSearchesLevel() > 0) ? new CString : NULL;
#endif

				pSearchTerms = CreateSearchExpressionTree(fileIO, 0);
				if (_pstrDbgSearchExpr)
				{
					TRACE(_T("KadSearchTerm=%s\n"), *_pstrDbgSearchExpr);
...
void CKademliaUDPListener::Process_KADEMLIA_PUBLISH_REQ (const byte *pbyPacketData, uint32 uLenPacket, uint32 uIP, uint16 uUDPPort)
{
	...
	bool bDbgInfo = true;//(thePrefs.GetDebugClientKadUDPLevel() > 0);
	CString sInfo;
	uint16 uCount = byteIO.ReadUInt16();
	bool bFlag = false;
	uint8 uLoad = 0;
...
//	fileIO2.WriteUInt16(thePrefs.GetPort());
	fileIO2.WriteUInt16(CKademlia::GetPrefs()->GetTcpPort());
//	if (thePrefs.GetDebugClientKadUDPLevel() > 0)
...
//	theApp.clientudp->SendPacket(pPacket, ntohl(uDestinationHost), uDestinationPort, false, NULL);  // kad doesnt supports obfuscation yet
	CKademlia::GetUDPSocket()->SendPacket(pPacket, ntohl(uDestinationHost), uDestinationPort, false, NULL);
}
