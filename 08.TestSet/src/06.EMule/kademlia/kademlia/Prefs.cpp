...
CPrefs::CPrefs()
{
	CString sFilename(CKademlia::m_strCofigPath);// = CMiscUtils::GetConfigDir();
...
void CPrefs::ReadFile()
{
	try
	{
		...
		if (file.Open(m_sFilename, CFile::modeRead | CFile::osSequentialScan | CFile::typeBinary | CFile::shareDenyWrite, &fexp))
		{
			...
			//--------------------------------------->
			m_uVersion = file.ReadUInt8();
			file.ReadUInt128(&m_uClientHash);
			//---------------------------------------<

			file.Close();

			struct in_addr addr;
			addr.S_un.S_addr = (ULONG)ntohl(m_uIP);
...
void CPrefs::WriteFile()
{
	try
	{
		...
		if (file.Open(m_sFilename, CFile::modeWrite | CFile::modeCreate | CFile::typeBinary | CFile::shareDenyWrite, &fexp))
		{
			...
			//--------------------------------------->
			file.WriteUInt8(m_uVersion);
			file.WriteUInt128(&m_uClientHash);
			file.WriteUInt8(0);
			//---------------------------------------<
...
//-- xt -- theApp.emuledlg->ShowConnectionState();
}
...
//-- xt --	theApp.serverlist->GetAvgFile( nServerAverage );
	uint32 nKadAverage = 0;//Kademlia::CKademlia::GetIndexed()->GetFileKeyCount();
