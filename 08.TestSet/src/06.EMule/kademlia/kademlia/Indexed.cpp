...
CIndexed::CIndexed()
{
	...
	m_sSourceFileName = CKademlia::m_strCofigPath;	//CMiscUtils::GetConfigDir();
	m_sSourceFileName.Append(_T("src_index.dat"));
	m_sKeyFileName = CKademlia::m_strCofigPath;		//CMiscUtils::GetConfigDir();
	m_sKeyFileName.Append(_T("key_index.dat"));
	m_sLoadFileName = CKademlia::m_strCofigPath;	//CMiscUtils::GetConfigDir();
...
void CIndexed::ReadFile(void)
{
	try
	{
		...
		if (fileSource.Open(m_sSourceFileName, CFile::modeRead | CFile::typeBinary | CFile::shareDenyWrite))
		{
			...
			TRACE(_T("Read %u source, %u keyword, and %u load entries\n"), uTotalSource, uTotalKeyword, uTotalLoad);
		}
	}
	catch ( CIOException *ioe )
	{
 		//AddDebugLogLine( false, _T("Exception in CIndexed::readFile (IO error(%i))"), ioe->m_iCause);
		TRACE(_T("Exception in CIndexed::readFile (IO error(%i))\n"), ioe->m_iCause);
		ioe->Delete();
	}
	catch (...)
	{
 		//AddDebugLogLine(false, _T("Exception in CIndexed::readFile"));
		TRACE(_T("Exception in CIndexed::readFile\n"));
	}
}
...
#pragma warning(disable : 4244)
	try
	{
		...
		TRACE(_T("Wrote %u source, %u keyword, and %u load entries\n"), uTotalSource, uTotalKey, uTotalLoad);
	...
	catch ( CIOException *ioe )
	{
		...
		TRACE(_T("Exception in CIndexed::~CIndexed (IO error(%i))\n"), ioe->m_iCause);
		ioe->Delete();
	}
	catch (...)
	{
 		//AddDebugLogLine(false, _T("Exception in CIndexed::~CIndexed"));
		TRACE(_T("Exception in CIndexed::~CIndexed\n"));
	}
...
void CIndexed::Clean(void)
{
	try
	{
		...
		TRACE(_T("Removed %u keyword out of %u and %u source out of %u\n"), uRemovedKey, uTotalKey, uRemovedSource, uTotalSource);
		m_tLastClean = time(NULL) + MIN2S(30);
	}
	catch(...)
	{
 		//AddDebugLogLine(false, _T("Exception in CIndexed::clean"));
		TRACE(_T("Exception in CIndexed::clean"));
		ASSERT(0);
	}
}
...
#pragma warning(default : 4244)
		...
		for (int iSearchTerm = 0; iSearchTerm < iStrSearchTerms; iSearchTerm++)
		{
			...
			if (wcsstr(pItem->m_fileName, CStringW(pSearchTerm->m_pastr->GetAt(iSearchTerm))) == NULL)
				return false;
...
//					if (thePrefs.GetDebugClientKadUDPLevel() > 0)
	...
	catch(...)
	{
		...
		TRACE(_T("Exception in CIndexed::SendValidNoteResult\n"));
	}
}
