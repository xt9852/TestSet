...
LPCTSTR CMiscUtils::GetConfigDir()
{
	if (m_strConfigDir.GetLength() == 0)
	{
		...
		::GetCurrentDirectory(MAX_PATH-1, m_strConfigDir.GetBufferSetLength(MAX_PATH));
		m_strConfigDir.ReleaseBuffer();
		//-----------------------------< xt
	}
...
void CMiscUtils::DebugHexDump(const byte *pbyData, uint32 uLenData)
{
	...
	try
	{
		...
		while (uPos < uLenData)
		{
			...
			//-- xt -- AddDebugLogLine(false, _T("%s"), sLine);
			uPos += iLenLine;
		}
	}
	catch (...)
	{
//-- xt -- 		AddDebugLogLine(false, _T("Exception in CMiscUtils::debugHexDump\n"));
	}
#else
