...
CKadTag *CDataIO::ReadTag(bool bOptACP)
{
	...
	catch (...)
	{
		...
		TRACE(_T("Invalid Kad tag; type=0x%02x  lenName=%u  name=0x%02x"), byType, uLenName, pcName!=NULL ? (BYTE)pcName[0] : 0);
		delete[] pcName;
		delete pRetVal;
		throw;
	}
...
void CDataIO::WriteUInt128(const CUInt128* pVal)
{
	WriteArray(pVal->GetData(), sizeof(uint32)*4);
}
...
void CDataIO::WriteTag(const CKadTag* pTag)
{
			...
			case TAGTYPE_STRING:
				{
					...
					TRACE(_T("CDataIO::WriteTag TAGTYPE_STRING"));
					break;
				}
			case TAGTYPE_UINT64:
	...
	{
//-- xt -- 		AddDebugLogLine( false, _T("Exception in CDataIO:writeTag (IO Error(%i))"), ioe->m_iCause);
		throw ioe;
	}
	catch (...)
	{
//-- xt -- 		AddDebugLogLine(false, _T("Exception in CDataIO:writeTag"));
		throw;
	}
}
...
{
	bool bResult = false;
	HRSRC hResInfo = FindResource(hInst, MAKEINTRESOURCE(IDR_EMULE_KAD_WIDECHARLOWERMAP), _T("WIDECHARMAP"));
	if (hResInfo)
	{
