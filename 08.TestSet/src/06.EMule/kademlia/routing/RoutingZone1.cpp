...
CRoutingZone::CRoutingZone()
{
	...
	m_sFilename = CKademlia::m_strCofigPath + _T("nodes.dat");
	// Init our root node.
...
/* xt				if (::IsGoodIPPort(uhostIP, uUDPPort))
						{
							...
							{
xt */						// This was not a dead contact, Inc counter if add was successful
...
// xt					}
			...
			TRACE(_T("Unable to read Kad file: %s"), m_sFilename);
	
	}
	catch (CFileException* e)
	{
		e->Delete();
		AddDebugError(_T("CFileException in CRoutingZone::readFile"));
	}
	// Show contact list in GUI
// xt		theApp.emuledlg->kademliawnd->ShowContacts();
		CKademlia::GetListCtrl()->ShowWindow(SW_SHOW);
		CKademlia::GetHistogramCtrl()->ShowWindow(SW_SHOW);
}
...
void CRoutingZone::WriteFile()
{
	...
	try
	{
		...
		CUInt128 uID;
		CBufferedFileIO file; //xt-> CSafeBufferedFile file;		
		CFileException fexp;
		if (file.Open(m_sFilename, CFile::modeWrite | CFile::modeCreate | CFile::typeBinary|CFile::shareDenyWrite, &fexp))
		{
			...
			file.Close();
			AddDebugLogLine( false, (_T("Wrote %ld contact%s to file.\n"), listContacts.size(), ((listContacts.size() == 1) ? _T("") : _T("s")));
		}
		else
			TRACE(_T("Unable to store Kad file: %s\n"), m_sFilename);
	}
	catch (CFileException* e)
	{
		e->Delete();
...
----------< xt */
	...
	AddUnfiltered(uID, uIP, uUDPPort, uTCPPort, uVersion, bUpdate);
