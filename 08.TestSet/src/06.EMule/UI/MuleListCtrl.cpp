...
bool _tmakepathlimit(TCHAR *path, const TCHAR *drive, const TCHAR *dir, const TCHAR *fname, const TCHAR *ext)
{
	...
	if (drive != NULL)
		nSize += (int)_tcsclen(drive);
	if (dir != NULL)
		nSize += (int)_tcsclen(dir);
	if (fname != NULL)
		nSize += (int)_tcsclen(fname);
	if (ext != NULL)
		nSize += (int)_tcsclen(ext);
...
void CMuleListCtrl::SetColors(LPCTSTR pszLvKey) {
	...
	CString strBkImage;
	LPCTSTR pszSkinProfile = _T("");//thePrefs.GetSkinProfile();
	...
	if(iColumn >= 0 && pHeaderCtrl->GetItem(iColumn, &headerItem)) {
		...
		if (hInstRes != NULL){
			...
			if (hbmSortStates != NULL){
				...
				if (imlSortStates.Create(14, 14, /*theApp.m_iDfltImageListColorFlags*/ ILC_COLOR32 | ILC_MASK, 1, 0)){
...
//						imlSortStates.SetBkColor(GetSysColor(COLOR_BTNFACE));
					...
					pHeaderCtrl->SetImageList(&imlSortStates);
					m_imlHeaderCtrl.DeleteImageList();
...
void CMuleListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) {
	...
	if (m_bCustomDraw) {
		if (bHighlight)
			crOldTextColor = pDC->SetTextColor(m_lvcd.clrText);//g_bLowColorDesktop ? m_crHighlightText : m_lvcd.clrText);
		else
