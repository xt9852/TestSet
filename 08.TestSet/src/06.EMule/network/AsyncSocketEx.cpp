...
CAsyncSocketEx::~CAsyncSocketEx()
{
	Close();
	//----------------------------------------
	if( m_bListSocket )	KillAllSockets();
	//----------------------------------------
...
void CAsyncSocketEx::OnReceive(int /*nErrorCode*/)
{
	...
	if( NULL == m_pBuffPool )
	{
		g_ProFile.m_pLog->println(LOG_ERROR, _T("OnReceive Error"));
		return;
	}

	PVOID pvBuf = m_pBuffPool->Get();

	if( SOCK_STREAM == m_nSocketType )
	{
		int ret = Receive( pvBuf, 4*1024 );
	...
	else
	{
		...
		int ret = ReceiveFrom( &Addr, iAddrLen, pvBuf, 4*1024 );
		if( ret == SOCKET_ERROR || ret == 0 )
		{
			return;
		}

		SendTo( &Addr, iAddrLen, pvBuf, ret );
	}

	m_pBuffPool->Free(pvBuf);
...
void CAsyncSocketEx::OnAccept(int /*nErrorCode*/)
{
	...
	SOCKET sNew = WSAAccept( m_SocketData.hSocket, (SOCKADDR*)&SockAddr, &iSockAddrLen, NULL, 0);
	...
	{
		CAsyncSocketEx *pnewclient = new CAsyncSocketEx;
		VERIFY( pnewclient->InitAsyncSocketExInstance() );
		pnewclient->m_SocketData.hSocket = sNew;
		pnewclient->AttachHandle(sNew);
		pnewclient->AsyncSelect(FD_WRITE | FD_READ | FD_CLOSE);
		pnewclient->SetBuffPool(m_pBuffPool);
	}
	//----------------------------------------
}
