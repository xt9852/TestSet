...
public:
	...
	BuffPool *m_pBuffPool;

	bool SetBuffPool(BuffPool *pBuffPool)
	{
	...
	CTypedPtrList<CPtrList, CAsyncSocketEx*> socket_list;

	UINT GetOpenSockets() {return socket_list.GetCount();}

	void AddSocket(CAsyncSocketEx* toadd) {socket_list.AddTail(toadd);}

	void RemoveSocket(CAsyncSocketEx* todel)
	{
	...
	void KillAllSockets()
	{
		...
		{
			CAsyncSocketEx* cur_socket = socket_list.GetAt(pos);
	...
	virtual int ReceiveFrom(SOCKADDR_IN *pAddr, int &nAddrLen, void* lpBuf, int nBufLen, int nFlags = 0)
	{
#ifndef NOLAYERS
		if (m_pFirstLayer)
...
#endif //NOLAYERS
			return recvfrom(m_SocketData.hSocket, (LPSTR)lpBuf, nBufLen, nFlags, (SOCKADDR*)pAddr, &nAddrLen);
	}
	virtual int SendTo(const SOCKADDR_IN *pAddr, const int nAddrLen, const void* lpBuf, int nBufLen, int nFlags = 0)
	{
#ifndef NOLAYERS
		if (m_pFirstLayer)
...
#endif //NOLAYERS
			return sendto(m_SocketData.hSocket, (LPSTR)lpBuf, nBufLen, nFlags, (SOCKADDR*)pAddr, nAddrLen);
	}
