...
Pinger::Pinger() {
    ...
    if (hICMP_DLL == 0) {
        TRACE(_T("Pinger: LoadLibrary() failed: Unable to locate ICMP.DLL!\n"));
        return;
    }
	...
	{
		 TRACE(_T("Pinger: GetProcAddr() failed for at least one function.\n"));
		return;
	}
    ...
    if (hICMP == INVALID_HANDLE_VALUE) {
        int nErr = GetLastError();
         TRACE(_T("Pinger: IcmpCreateFile() failed, err: %u\n"), nErr);
        PIcmpErr(nErr);
        return;
    }
...
Pinger::~Pinger() {
    ...
    if (fRet == FALSE) {
        int nErr = GetLastError();
        TRACE(_T("Error closing ICMP handle, err: %u\n"), nErr);
        PIcmpErr(nErr);
    }
   ...
   m_time.Tick();
	...
	while((usResTime += m_time.Tick()) < TIMEOUT){
		...
		if (((icmphdr->type == ICMP_TTL_EXPIRE) || (icmphdr->type == ICMP_DEST_UNREACH)) &&
			...
			if(doLog) {
				TRACE(_T("Reply (UDP-pinger) from %s: bytes=%d time=%3.2fms TTL=%i\n"),
					ipstr(stDestAddr),
					nRet,
					usResTime,
					returnValue.ttl);
			...
			//}
			if(doLog) {
				TRACE(_T("Filtered reply (UDP-pinger) from %s: bytes=%d time=%3.2fms TTL=%i type=%i\n"),
					ipstr(stDestAddr),
					nRet,
					usResTime,
					64 - (reply->ttl & 63),
...
PingStatus Pinger::PingICMP(uint32 lAddr, uint32 ttl, bool doLog) {
    ...
    if (dwReplyCount != 0) {
        ...
        if(doLog) {
           TRACE(_T("Reply (ICMP-pinger) from %s: bytes=%d time=%3.2fms (%3.2fms %ldms) TTL=%i\n"),
                                                        ipstr(stDestAddr),
        ...
        returnValue.error = lastError;
        if(doLog) {
			TRACE(_T("Error from %s: Error=%i"),ipstr(stDestAddr),
				returnValue.error);
        }
    }
