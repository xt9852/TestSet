...
#define read4(p, a)  rmff_get_uint32_be( (unsigned char *)&((p)->a) )
#define read2(p, a)  rmff_get_uint16_be( (unsigned char *)&((p)->a) )
int rmff_read_headers(rmff_file_t *file) 
{
 ...
 int  len;
  ...
  //add by xianjian at 3.28 for jinzhiyulie.rm
  len = read4( &rmf->obj, size);
  if(len == 18 )
  {
...
//读下一个Frame的头部
//
rmff_frame_t * rmff_read_next_frame_header(rmff_file_t *file, void *buffer) 
{
	rmff_frame_t *frame;
	rmff_file_internal_t *fint;
	uint16_t object_version, length;
	...
	if (object_id == rmffFOURCC('D', 'A', 'T', 'A')) {
		...
		fint->next_data_offset = read_uint32_be();
		file->num_packets_read = 0;
		return rmff_read_next_frame_header(file, buffer);
	}
	...
	if(io->seek(fh,frame->size,SEEK_CUR) != 0){
		rmff_release_frame(frame);
		return (rmff_frame_t *)set_error(RMFF_ERR_EOF, NULL, 0);
	}
...
//inline 
uint16_t data_get_uint16_be(unsigned char **data,    int *len) 
{
  (*data) += 2;
  (*len) -= 2;
...
//inline 
unsigned char data_get_uint8(unsigned char **data,   int *len) 
{
  (*data)++;
  (*len)--;
  return *((*data) - 1);
