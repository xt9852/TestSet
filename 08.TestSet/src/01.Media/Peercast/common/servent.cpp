...
void Servent::initIncoming(ClientSocket *s, unsigned int a)
{
	...
	{
		//LOG_ERROR1("!!FATAL!! Incoming error: %s",e.msg);
		//servMgr->shutdownTimer = 1;  	
		kill();

		LOG_ERROR1("INCOMING FAILED: %s",e.msg);
...
void Servent::initOutgoing(TYPE ty)
{
	...
	{
		LOG_ERROR1("Unable to start outgoing: %s",e.msg);
		kill();
	}
}
...
void Servent::initPCP(Host &rh)
{
	...
	{
		LOG_ERROR1("Unable to open connection to %s - %s",ipStr,e.msg);
		kill();
	}
}
...
void Servent::initGIV(Host &h, GnuID &id)
{
	...
	{
		LOG_ERROR1("GIV error to %s: %s",ipStr,e.msg);
		kill();
	}
}
// -----------------------------------
...
{
	if (sock)
		LOG_ERROR1("Servent::createSocket attempt made while active");
...
void Servent::handshakeOut()
{
	...
	if (r != 200)
	{
		LOG_ERROR1("Expected 200, got %d",r);
		...
		}else
		{
			...
			} else if (outputProtocol == ChanInfo::SP_MMS)
			{
				...
				}else
				{
					...
					if (ch)
						sock->writeLineF("StuffString( CRLF );: %d",ch->headPack.len);
...
void Servent::handshakeOutgoingPCP(AtomStream &atom, Host &rhost, GnuID &rid, String &agent, bool isTrusted)
{
	...
	if (isTrusted)
	{
		...
		if (disable == 1)
		{
			LOG_ERROR1("client disabled: %d",disable);
...
void Servent::processIncomingPCP(bool suggestOthers)
{
	...
	if (unavailable || alreadyConnected || offair)
	{
		...
		LOG_ERROR1("Sending QUIT to incoming: %d",error);
	...
	while (sv->thread.active)
	{
		...
		if (chanMgr->isBroadcasting() && servMgr->autoServe)
		{
			...
			try 
			{
				...
				LOG_ERROR1("COUT to %s closed: %d",ipStr,error);

			}catch(TimeoutException &e)
			{
				LOG_ERROR1("COUT to %s: timeout (%s)",ipStr,e.msg);
				sv->setStatus(S_TIMEOUT);
			}catch(StreamException &e)
			{
				LOG_ERROR1("COUT to %s: %s",ipStr,e.msg);
...
//	thread->lock();
	...
	}catch(HTTPException &e)
	{
		...
		LOG_ERROR1("Incoming from %s: %s",ipStr,e.msg);
	}catch(StreamException &e)
	{
		LOG_ERROR1("Incoming from %s: %s",ipStr,e.msg);
	}
...
void Servent::ProcessStreamEx(bool doneHandshake,ChanInfo &chanInfo)
{
...
void Servent::sendRawChannel(bool sendHead, bool sendData)
{
				...
				if (ch)
				{
					...
					if (ch->rawData.findPacket(streamPos,rawPack))
					{
						if (syncPos != rawPack.sync)
							LOG_ERROR1("Send skip: %d",rawPack.sync-syncPos);
	...
	{
		LOG_ERROR1("Stream channel: %s",e.msg);
	}
}
...
//	thread->lock();
	...
	{
		LOG_ERROR1("Server Error: %s:%d",e.msg,e.err);
	}
