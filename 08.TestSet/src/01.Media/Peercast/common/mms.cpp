...
extern char pasfHead[16384];
extern int  asflen;
extern unsigned int streampos;
...
{
	{
		...
		switch (chunk.type)
		{
			case 0x4824:		// asf header
			{
                ...
                memset(&pasfHead[0],0,16384);
				memcpy(&pasfHead[0],&ch->headPack.data[0],ch->headPack.len);
				asflen = ch->headPack.len;
				ch->streamPos += ch->headPack.len;
				
				//SendAllUserAsfHead(&pasfHead[0],asflen);
			...
			case 0x4424:		// asf data
			{
                ...
                //SendAllUserData(&pack.data[0],pack.len,pack.pos);
