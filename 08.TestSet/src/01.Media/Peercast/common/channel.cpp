...
ChanPacketBuffer	HttprawData;


//extern CMsgProcess g_Process;
// -----------------------------------
...
void	Channel::newPacket(ChanPacket &pack)
{
	if (pack.type != ChanPacket::T_PCP)
	{
		...
		HttprawData.writePacket(pack,true);
	}
}
	...
	while (ch->thread.active)
	{
		...
		if (ch->sourceHost.host.ip)
		{
			...
			}catch(StreamException &e)
			{
				...
				LOG_ERROR1("Channel to %s %s : %s",ipstr,type,e.msg);
...
bool	Channel::checkBump()
{
	if (!isBroadcasting() && (!sourceHost.tracker))
		...
		{
			LOG_ERROR1("Channel Auto bumped");
			bump = true;
		}
...
bool ChanInfo::update(ChanInfo &info)
{
	...
	if (bcID.isSet())
	{
		...
		{
			LOG_ERROR1("ChanInfo BC key not valid");
			return false;
		}
	}else
	{
