...
#include "stdafx.h"
#include "MeidaCapture.h"


CMediaCapt::CMediaCapt()
    :m_bAudio(FALSE),
    m_bVideo(FALSE),
    m_hDib(NULL),
    m_pBmpHeader(NULL),
...
void CMediaCapt::Init(CAVSend *pAVSend)
{
    m_pAVSend = pAVSend;
    m_hDib = ::DrawDibOpen();
...
void CMediaCapt::AddShow(HDC hDC, int x, int y)
{
...
void CMediaCapt::DelShow(HDC hDC)
{
    m_mapHdc.erase(hDC);
}

void CMediaCapt::StartSaveFile(LPCTSTR psFile)
{
    if (!m_bSaveFile)
    {
...
void CMediaCapt::StopSaveFile(void)
{
    if (m_bSaveFile)
    {
        m_File.Close();
...
void CMediaCapt::Start(BOOL bAudio, BOOL bVideo)
{
    m_bAudio = bAudio;
    m_bVideo = bVideo;
...
void CMediaCapt::Stop(BOOL bAudio, BOOL bVideo)
{
    m_bAudio = !bAudio;
    m_bVideo = !bVideo;
...
void CMediaCapt::ProcAudio(char *pszData, DWORD dwLen)
{
    int nOutLen = 0;
...
void CMediaCapt::ProcVideo(LPBYTE pbyData, int nLen)
{
