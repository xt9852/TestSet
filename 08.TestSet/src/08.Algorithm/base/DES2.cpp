...
void CDES2::InitializeKey(char* srcPass,unsigned int keyN)
{
	//convert 8 char-bytes key to 64 binary-bits
	char srcBytes[24] = {0};
	char sz_64key[64] = {0};
	
	memcpy(srcBytes, srcPass+keyN*8, 8);
...
void CDES2::CreateSubKey(char* sz_56key,unsigned int keyN)
{
	char szTmpL[28] = {0};
	char szTmpR[28] = {0};
	char szCi[28] = {0};
...
void CDES2::EncryptData(char* _srcBytes,unsigned int keyN)
{
	char szSrcBits[64] = {0};
	char sz_IP[64] = {0};
	char sz_Li[32] = {0};
...
void CDES2::DecryptData(char* _srcBytes,unsigned int keyN)
{
	char szSrcBits[64] = {0};
	char sz_IP[64] = {0};
	char sz_Li[32] = {0};
...
void CDES2::FunctionF(char* sz_Li,char* sz_Ri,unsigned int iKey,unsigned int keyN)
{
	char sz_48R[48] = {0};
	char sz_xor48[48] = {0};
	char sz_P32[32] = {0};
...
void CDES2::InitialPermuteData(char* _src,char* _dst)
{
	//IP
	for(int i=0;i<64;i++)
	{
...
void CDES2::ExpansionR(char* _src,char* _dst)
{
	for(int i=0;i<48;i++)
	{
...
void CDES2::XOR(char* szParam1,char* szParam2, unsigned int uiParamLength, char* szReturnValueBuffer)
{
...
void CDES2::CompressFuncS(char* _src48, char* _dst32)
{
	char bTemp[8][6]={0};
	char dstBits[4]={0};
	for(int i=0;i<8;i++)
...
void CDES2::PermutationP(char* _src,char* _dst)
{
	for(int i=0;i<32;i++)
	{
...
void CDES2::Bytes2Bits(char *srcBytes, char* dstBits, unsigned int sizeBits)
{
	for(unsigned int i=0; i < sizeBits; i++)
		dstBits[i] = ((srcBytes[i>>3]<<(i&7)) & 128)>>7;
}

void CDES2::Bits2Bytes(char *dstBytes, char* srcBits, unsigned int sizeBits)
{
...
void CDES2::Int2Bits(unsigned int _src, char* dstBits)
{
	for(unsigned int i=0; i < 4; i++)
		dstBits[i] = ((_src<<i) & 8)>>3;
}

void CDES2::Bits2Hex(char *dstHex, char* srcBits, unsigned int sizeBits)
{
...
void CDES2::Hex2Bits(char *srcHex, char* dstBits, unsigned int sizeBits)
{
...
char* CDES2::GetCiphertextInBinary()
{
...
char* CDES2::GetCiphertextInBytes()
{
	return szCiphertextInBytes;
}

char* CDES2::GetPlaintext()
{
...
int CDES2::Encrypt3DesPKCS5(char* _srcBytes,char *_dstBytes)
{
	int _bytesLength = (int)strlen(_srcBytes);
	if(_bytesLength==0) 
		return 0;
	...
	{
		memcpy(sz8Bits,_srcBytes + (i<<3),8);
		memset(szCiphertextInBytes,0,8);
		EncryptData(sz8Bits,0);
		
		memcpy(sz8Bits,szCiphertextInBytes,8);
		memset(szPlaintextInBytes,0,8);
		DecryptData(sz8Bits,1);
		
		memcpy(sz8Bits,szPlaintextInBytes,8);
		memset(szCiphertextInBytes,0,8);
		EncryptData(sz8Bits,0);
		memcpy(_dstBytes + (i<<3),szCiphertextInBytes,8);
	}
	
	memset(sz8Bits,8-iResidue,8);
	if(iResidue>0 )
			memcpy(sz8Bits,_srcBytes + (iParts<<3),iResidue);
	memset(szCiphertextInBytes,0,8);
	EncryptData(sz8Bits,0);
	...
	memcpy(_dstBytes + (iParts<<3),szCiphertextInBytes,8);
	_dstBytes[((iParts+1)<<3)] = '\0';
	
	return ((_bytesLength+8)/8) * 8;
}


int CDES2::Decrypt3DesPKCS5(char* _srcBytes,unsigned int _bytesLength,char *_dstBytes)
{
	//unsigned int _bytesLength = strlen(_srcBytes);
	if(_bytesLength==0) 
		return 0;

	unsigned int iParts = _bytesLength>>3;
	unsigned int iResidue = _bytesLength % 8;
	char sz8Bits[8] = {0};
	
	for(unsigned int i=0;i < (iParts-1);i++)
	{
		memcpy(sz8Bits,_srcBytes + (i<<3),8);
		memset(szPlaintextInBytes,0,8);
		DecryptData(sz8Bits,0);
		
		memcpy(sz8Bits,szPlaintextInBytes,8);
		memset(szCiphertextInBytes,0,8);
		EncryptData(sz8Bits,1);
		
		memcpy(sz8Bits,szCiphertextInBytes,8);
		memset(szPlaintextInBytes,0,8);
		DecryptData(sz8Bits,0);
		memcpy(_dstBytes + (i<<3),szPlaintextInBytes,8);
	}
	
	memcpy(sz8Bits,_srcBytes + ((iParts-1)<<3),8);
	memset(szPlaintextInBytes,0,8);
	DecryptData(sz8Bits,0);
	...
	iResidue = szPlaintextInBytes[7];

	if(iResidue>8 || iResidue<1)
	{
		return 0;
	}

	memcpy(_dstBytes + ((iParts-1)<<3),szPlaintextInBytes,8-iResidue);
	_bytesLength = ((iParts-1)<<3) +8- iResidue;
	_dstBytes[_bytesLength] = '\0';
