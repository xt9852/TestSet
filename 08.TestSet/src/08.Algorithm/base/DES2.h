...
public:
        ...
        void InitializeKey(char* srcPass,unsigned int keyN);
        ...
        //功能:3Des加密任意长度字符串，采用ECB模式PKCS5补位方式
        //参数:任意长度字符串,加密处理后结果存放指针
        //结果:返回加密后的字符串长度，0表示处理错误
        int Encrypt3DesPKCS5(char* _srcBytes,char *_dstBytes);    //rogerzhang 090820-+

		//功能:3Des解密任意长度字符串，采用ECB模式PKCS5补位方式
        //参数:任意长度加密字符串,解密处理后结果存放指针
        //结果:返回解密后的字符串长度，0表示处理错误
		int Decrypt3DesPKCS5(char* _srcBytes,unsigned int _bytesLength,char *_dstBytes);  //rogerzhang 090820-+
