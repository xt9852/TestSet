...
CXTMenu::CXTMenu()
    :m_nWidth(25),      // default menu width
    m_nHeight(32),       // default menu height
...
BOOL CXTMenu::AppendMenu(UINT nFlags, UINT nIdItem /*= 0*/, LPCTSTR pstrItem /*= NULL*/, INT nImageIndex/* = -1*/)
{
    ...
    CMenuItem* pItem = new CMenuItem(nIdItem, m_nHeight, nImageIndex);
...
void CXTMenu::DrawXPMenu(LPDRAWITEMSTRUCT lpDIS)
{
    ...
    if ((lpDIS->itemAction & ODA_DRAWENTIRE) || (!(lpDIS->itemState & ODS_SELECTED) && (lpDIS->itemAction & ODA_SELECT)))
    {
        ...
        pDC->DrawText(pItem->m_tszText, (int)_tcslen(pItem->m_tszText), rcText, 
            DT_LEFT|DT_SINGLELINE|DT_WORD_ELLIPSIS|DT_CALCRECT);

        m_nWidth = rcText.right;
