...
COScopeCtrl::COScopeCtrl(int NTrends)
{
	...
	m_str.YUnits.Format(_T(""));         // can also be set with SetYUnits
...
int COScopeCtrl::FontPointSizeToLogUnits(int nPointSize)
{
    HDC hDC = ::GetDC(NULL);
    if (hDC)
    {
...
void COScopeCtrl::InvalidateCtrl(bool deleteGraph)
{
	...
	COLORREF crLabelBk;
	COLORREF crLabelFg;
	if (/*thePrefs.GetStraightWindowStyles()*/STYLES) {
	...
	if (1/*STYLESthePrefs.m_bShowVerticalHourMarkers*/)
	{
...
/*
    if (theApp.emuledlg->IsRunning()) 
	{
		if (!thePrefs.IsGraphRecreateDisabled())
		{
			...
			VERIFY( (m_nRedrawTimer = (UINT)SetTimer(1612, 200, NULL)) != NULL ); // reduce flickering
		}
	}
*/
...
void COScopeCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (pwndParent)
	{
		...
		int shownsecs = plotRect.Width() * 1;//thePrefs.GetTrafficOMeterInterval();
